...
export const PLAYER_TOKEN_BUDGET = 80000;
...
export class ModelGate {
  ...
  acquire(playerId: string, reservation: number, playerSpent: number, roomSpent: number, now = Date.now()) {
    if (this.activePlayers.has(playerId)) throw new RequestError('Luna is already choosing your move.', 'busy', 1000);
    const remaining = AI_COOLDOWN_MS - (now - (this.lastRequest.get(playerId) ?? -Infinity));
    ...
    if (reservation > 16000 || playerSpent + reservation > PLAYER_TOKEN_BUDGET || roomSpent + this.reserved + reservation > ROOM_TOKEN_BUDGET) throw new RequestError('The model token budget is reached. Manual play is still available.', 'budget');
    this.inFlight += 1;
...
export class LunaGateway implements ModelGateway {
  ...
  async complete(game: Game, options: AiOptions, cacheBucket: string): Promise<Insight> {
    ...
    const request: OpenAI.Chat.ChatCompletionCreateParamsNonStreaming = {
      ...
      store: false,
      prompt_cache_key: `tokenfall-policy-v1:${cacheBucket}`,
