...
- **MCP** enables real two-piece lookahead before the next Luna request. It simulates each current move and the legal next placements, including Hold, so Luna can compare next-turn traps, achievable line clears, remaining holes, and stack height. It defaults off and survives reloads. The labeled **Results** button opens the forecasts, actual tool arguments, and estimated extra input cost. Opening the popup pauses play and stops Luna; later replies do not replace the selected result.
